...
function eliminarProveedor($id){
    $sentencia = "DELETE FROM proveedores WHERE id = ?";
    return eliminar($sentencia, $id);
}

function editarProveedor($marca, $nombre, $telefono, $direccion, $id){
    $sentencia = "UPDATE proveedores SET marca = ?, nombre = ?, telefono = ?, direccion = ? WHERE id = ?";
    $parametros = [$marca, $nombre, $telefono, $direccion, $id];
    return editar($sentencia, $parametros);
}

function obtenerProveedorPorId($id){
    $sentencia = "SELECT id, marca, nombre, telefono, direccion FROM proveedores WHERE id = ?";
    return select($sentencia, [$id])[0];
}

function obtenerProveedores(){
    $sentencia = "SELECT id, marca, nombre, telefono, direccion FROM proveedores";
    return select($sentencia);
}

function registrarProveedores($usuario, $nombre, $telefono, $direccion){
    $sentencia = "INSERT INTO proveedores (marca, nombre, telefono, direccion) VALUES (?,?,?,?)";
    $parametros = [$marca, $nombre, $telefono, $direccion];
    return insertar($sentencia, $parametros);
}
